...
- 当前阶段：Phase 8 重构已完成，进入 Phase 9 Beta 稳定与产品化阶段。
- 当前版本：`3.0.0-beta.1`；正式版 `3.0.0` 必须等 Phase 9 必需任务全部完成后再发布。
- 当前任务：RF-910 前端 Beta 收束已完成；下一次 `main` push 验证 Workers Builds，然后进入 RF-911 生产回归矩阵。
- 阻塞项：本地实现完成后需要维护者在现有 Worker 连接官方仓库并执行一次真实 Cloudflare Build smoke。Turnstile 与 Workers observability 按决策暂不启用。
- 当前生产已运行 Beta 版本；Worker 仅绑定 R2，不再绑定 KV。
...
| 9 Beta 稳定与产品化 | IN_PROGRESS | 部署方案、目标适配器、前端优化、Beta 验收与正式版门禁 |
...
| ID | 状态 | 任务 | 验证 |
|---|---|---|---|
| RF-900 | IN_PROGRESS | 实现 fork + Cloudflare Workers Builds 从零部署 | unit + real Cloudflare smoke |
| RF-901 | DONE | 自动上传管理员 Build Secret 并生成缺失 signing secrets | secret lifecycle/redaction tests |
| RF-902 | IN_PROGRESS | 网站部署移出 GitHub Action，维护者与用户统一跟踪各自 `main` | workflow/config review |
| RF-903 | SKIPPED | 不实现独立 Release CI/manifest/checksum 分发 | ADR-047 |
| RF-904 | IN_PROGRESS | 自动创建/复用 R2、启用 workers.dev 并验证 bootstrap | unit + fresh-account smoke |
| RF-905 | IN_PROGRESS | 使用 Sync fork 更新与 Cloudflare Worker version rollback | operations review + rehearsal |
...
| RF-911 | TODO | 建立 Beta 反馈、P0/P1 修复和生产回归矩阵 | issue matrix + verify |
| RF-912 | TODO | 正式版 `3.0.0` 发布门禁、Release smoke 和回滚复核 | release review |
...
- 待生产验证：RF-900/RF-902/RF-904/RF-905 保持 `IN_PROGRESS`。需要在现有 `sing-sub` Worker 连接官方仓库并完成首次 Build、业务 smoke 和一次版本回滚演练；这些步骤需要维护者 Cloudflare 控制台权限。
- 兼容边界：旧登录导入和历史迁移代码已直接删除；生产验证后没有待删除的一次性兼容代码。部署初始化器是长期正式入口。

### 2026-07-16：RF-910 前端 Beta 优化完成

- 弹窗：名称与备注改为明确的垂直字段，移除可见 `.json` 后缀；预览态保持名称主层级和备注辅助层级；编辑/预览、保存与关闭在状态切换时不跳位。
...
- 追加收束：侧栏激活项改为持久主题色背景和左侧强调，悬停只使用中性背景；同步页刷新/拉取/推送共享互斥忙碌态，显示当前操作文案与加载图标，请求和后续状态刷新完成前禁止重复操作。
- 验证：`npm run verify` 全通过（107 unit、66 integration、18/18 desktop/mobile E2E）；浏览器复核覆盖 320/390/412/768/1024/1440 宽度和中英文；同步慢响应与强制重复点击回归通过；`npm run worker:dry-run` 通过，总上传 749.93 KiB / gzip 122.22 KiB，仅绑定 `WORKSPACE_BUCKET`，未部署生产。
- 下一步：通过下一次 `main` push 同时验证新连接的 Cloudflare Workers Builds，完成后进入 RF-911 Beta 生产回归。
