...
- 当前任务：部署方案、Profile 派生/补丁机制优化、前端 Beta 反馈和正式版验收门禁。
...
| RF-902 | DONE | production deploy 限制 main ref 并先执行 Worker dry-run | workflow review |
...
| RF-908 | TODO | 设计 target adapter 与 Momo typed options，评估 patch/overrides 合并 | ADR + sample configs |
| RF-909 | TODO | 实现目标适配器、迁移旧 patch 并删除重复日常覆盖入口 | builder/schema/UI tests |
...
- 配置方向：评估用 `native`/`momo` target adapter 合并 `overrides` 与 patch 的重复语义；待真实 Momo 样例确认后再接受 ADR-043 和实施迁移。
- 前端方向：继续收集生产 Beta 使用反馈，优先修复功能、移动端、i18n、可访问性和性能问题，不重开基础架构或组件库迁移。
- 下一任务：收集 Momo 当前基础模板、现有 patch 和期望最终配置，完成 RF-908 设计；部署助手与前端问题可作为并行的独立 Beta 任务推进。
