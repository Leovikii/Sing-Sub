...
- 弹窗：桌面名称与备注改为带最小宽度的动态同排字段，模式切换紧邻保存左侧；中等宽度分离元数据/操作行，手机端保持垂直字段；移除可见 `.json` 后缀，所有状态切换不跳位。
...
- 元数据：关于页许可证从错误的硬编码 GPL-3.0 修正为可查看仓库正文的 MIT 链接，与根目录 `LICENSE` 和 `package.json` 一致。
