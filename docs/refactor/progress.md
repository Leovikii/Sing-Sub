...
- 当前任务：部署方案、前端 Beta 反馈和正式版验收门禁；Profile replacement adapter 已完成。
...
- 兼容：按产品决策不实现旧 workspace、旧 Profile、旧 GitHub patches 或旧 DSL 读取/转换。生产首次切换必须重新初始化 workspace，或部署前一次性重新导入新格式；只删除旧 patch 文件不足以升级现有 R2 revision。
