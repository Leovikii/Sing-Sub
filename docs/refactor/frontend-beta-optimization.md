...
| FBO-14 | DONE | 压缩桌面编辑弹窗标题区 | 名称/备注使用有界弹性宽度；预览内容紧凑组合；移动端保持纵向字段 |
| FBO-15 | DONE | 对齐关于页许可证元数据 | 显示 GPL-3.0 并链接仓库 `LICENSE` |

## 编辑弹窗

桌面使用紧凑稳定布局：`metadata 1fr | mode auto | actions auto`。编辑态名称在 160-224px、备注在 224-320px 范围内弹性收缩，不填满 metadata 的剩余宽度；预览态名称与备注在左侧按内容宽度紧凑组合。模式切换紧邻保存左侧，保存和关闭靠右。中等宽度先把操作区折到第二行，手机端再让名称和备注恢复为纵向满宽字段。

- 编辑态使用有明确 label 的名称和备注输入；备注不显示“可选”等括号补充。
- 名称仅显示用户可编辑部分，不在弹窗标题尾部拼接 `.json`。
- 预览态名称使用主文本，备注以 16px 间距紧跟其后并作为弱化辅助文本；空备注不渲染占位文案。
...
- 追加 FBO-14/FBO-15：桌面编辑弹窗标题区压缩为有界单行元数据和右侧连续操作区，预览名称/备注按内容紧凑组合；关于页许可证与根目录、npm metadata 的 GPL-3.0-only 声明一致。
