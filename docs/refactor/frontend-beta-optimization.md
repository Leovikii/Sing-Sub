...
| FBO-16 | DONE | 收紧并稳定编辑弹窗元数据轨道 | 字段更紧凑；预览文本放大居中；编辑/预览切换不改变 header 高度或内容起点 |

## 编辑弹窗

桌面使用紧凑稳定布局：`metadata 1fr | mode auto | actions auto`。编辑态名称在 112-176px、备注在 128-224px 范围内弹性收缩，不填满 metadata 的剩余宽度；预览态名称与备注在左侧按内容宽度紧凑组合。桌面两态共用 56px 元数据轨道，手机端两态共用 112px 轨道，切换不得改变 header 高度或正文起点。模式切换紧邻保存左侧，保存和关闭靠右。中等宽度先把操作区折到第二行，手机端再让名称和备注恢复为纵向满宽字段。

- 编辑态使用有明确 label 的名称和备注输入；备注不显示“可选”等括号补充。
- 名称仅显示用户可编辑部分，不在弹窗标题尾部拼接 `.json`。
- 预览态名称使用 20px 主文本，备注使用 16px 弱化辅助文本并以 16px 间距紧跟其后；两者在元数据轨道内垂直居中，空备注不渲染占位文案。
...
- 追加 FBO-16：进一步收紧名称/备注宽度，放大并居中预览文本；桌面和手机端分别固定共享元数据轨道，E2E 同时约束 header 高度、正文起点、模式控件和保存按钮不位移。
