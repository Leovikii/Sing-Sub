...
- Beta 后续简化目标：普通用户运行本地部署助手，选择 Worker/R2/域名并输入管理员口令；助手在本机内存生成并上传两个 signing secret。部署后 WebUI 默认只创建空 `primary` workspace，GitHub 在仓库设置中后连。该流程已进入 Phase 9，当前初始化代码暂不修改。
