...
const RULESET_METADATA_ALLOWED_KEYS = new Set(['note', 'sources']);
const RULESET_SOURCE_ALLOWED_KEYS = new Set(['url', 'interval_hours', 'last_updated', 'rules']);
...
function validateRulesetSource(content: string): string | null {
  ...
  for (const rule of data.rules) {
    if (!rule || typeof rule !== 'object' || Array.isArray(rule)) return 'Rule-set rules must be objects';
    ...
    if (entries.length !== 1 || (entries[0][0] !== 'domain' && entries[0][0] !== 'domain_suffix')) {
      return 'Rule-set rules may only contain domain or domain_suffix fields';
    }
    try {
  ...
  if (metadata !== undefined) {
    ...
    if (metadataRecord.note !== undefined && typeof metadataRecord.note !== 'string') return `${RULESET_METADATA_KEY}.note must be a string`;
    if (metadataRecord.sources !== undefined) {
      ...
      for (const source of metadataRecord.sources) {
        if (!source || typeof source !== 'object' || Array.isArray(source)) return 'Rule-set source metadata must be an object';
        ...
        if (entry.last_updated !== undefined && typeof entry.last_updated !== 'string') return 'Rule-set source last_updated must be a string';
        const sourceRules = entry.rules;
        if (sourceRules !== undefined && !Array.isArray(sourceRules)) return 'Rule-set source rules must be an array';
        for (const rule of sourceRules || []) {
          if (!rule || typeof rule !== 'object' || Array.isArray(rule)) return 'Rule-set source rules must be objects';
          const entries = Object.entries(rule as Record<string, unknown>);
          if (entries.length !== 1 || (entries[0][0] !== 'domain' && entries[0][0] !== 'domain_suffix')) {
            return 'Rule-set source rules may only contain domain or domain_suffix fields';
          }
          try {
            normalizeImportedRuleValues(entries[0][1], entries[0][0] as 'domain' | 'domain_suffix');
          } catch (error: any) {
...
const LEGACY_COMPILE_SRS_WORKFLOW_CONTENT = `name: Compile SRS
on:
  push:
    paths:
  ...
  if (isRuleset) {
    ...
    try {
      ...
      await commitMultiFiles(
        session,
        treeItems,
        workflowNeedsSync ? 'ruleset: synchronize compiler and update source' : actionMessage,
      );
    } catch (e) {
