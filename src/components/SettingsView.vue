<template>
  <div class="max-w-3xl mx-auto space-y-5 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="glass border border-border-base rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-4 md:gap-6 relative overflow-hidden">
      ...
      <div class="relative">
        <img v-if="user" :src="user.avatar_url" class="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-bg-surface shadow-xl" />
        <div class="absolute bottom-0 right-0 w-5 h-5 md:w-6 md:h-6 bg-emerald-500 rounded-full border-[3px] md:border-4 border-bg-surface"></div>
      </div>

      <div class="text-center md:text-left z-10 flex-1">
        <h2 class="text-xl md:text-2xl font-bold text-text-primary">{{ user?.login || '未登录' }}</h2>
        <p class="text-text-muted mt-1 flex items-center justify-center md:justify-start gap-1.5 text-sm md:text-base">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
      ...
      <div class="glass border border-border-base rounded-3xl p-5 md:p-6 space-y-4 md:space-y-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-base md:text-lg font-semibold text-text-primary">仓库设置</h3>
            <p class="text-xs md:text-sm text-text-muted">配置数据存储位置</p>
          </div>
        </div>
        ...
        <div class="space-y-3 md:space-y-4">
          ...
          <div>
            ...
            <p class="text-[11px] md:text-xs text-text-muted mt-1.5 md:mt-2">用于通过 GitHub API 读写配置文件</p>
          </div>
        </div>
      </div>
      ...
      <div class="glass border border-border-base rounded-3xl p-5 md:p-6 space-y-4 md:space-y-6 flex flex-col">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-base md:text-lg font-semibold text-text-primary">订阅访问</h3>
            <p class="text-xs md:text-sm text-text-muted">管理客户端拉取权限</p>
          </div>
        </div>
        ...
        <div class="space-y-3 md:space-y-4 flex-1">
          <div>
            ...
            <div class="flex gap-2">
              ...
              <button
                @click="generateToken"
                class="shrink-0 px-3 md:px-4 rounded-xl bg-bg-elevated text-text-muted hover:text-brand-pink hover:bg-bg-hover transition-colors cursor-pointer border border-border-base"
                title="随机生成"
              >
        ...
        <div class="pt-2 md:pt-4 flex justify-end">
          <Button @click="handleSave" :loading="loading" variant="primary" class="px-6 md:px-8 w-full md:w-auto shadow-lg shadow-brand-pink/20">
            保存更改
          </Button>
        </div>
