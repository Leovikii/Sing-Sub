<template>
  <div class="flex items-center gap-1 p-2 bg-bg-surface border-b border-border-base overflow-x-auto hide-scrollbar shrink-0">
    ...
    <div class="flex items-center gap-1 bg-bg-code-toolbar rounded-md p-1 border border-bg-elevated">
      ...
      <button @click="$emit('redo')" :disabled="!canRedo" class="flex items-center gap-2 px-2.5 py-1.5 rounded-sm text-text-muted hover:text-text-primary hover:bg-bg-elevated transition-colors disabled:opacity-30 disabled:cursor-not-allowed shrink-0" title="重做 (Ctrl+Y)">
        <Redo2 class="w-4 h-4" />
